angular.module('alaskaIonic').controller('findFlightsCtrl',function ($scope,$state,flightsSrvc,$http){
	...
	$scope.findFlights = function() {
        ...
        if(oneWay===0){
            var returnDate=new Date($scope.onezoneDatepickerReturn.date).getTime();
        }
        $state.go('tabsController.findFlights.flights');
        // console.log(onezoneDatepicker.startDate);
                /*one way trip*/
        // if(oneWay===1){
        //     $http.get('localhost:8100/api/flights/search/'+origin+'/'+destination+'/'+departingDate+'/'+seatClass).success(function(flights){
        //         flightsSrvc.outgoingFlights.concat(flights);
        //         if(otherAirlines===1){
        //             $http.get('localhost:8100/api/other/flights/search/'+origin+'/'+destination+'/'+departingDate+'/'+seatClass).success(function(othersFlights){
        ...
        //     $http.get('localhost:8100/api/flights/search/'+origin+'/'+destination+'/'+departingDate+'/'+returningDate+'/'+seatClass).success(function(flights){
        //         flightsSrvc.outgoingFlights.concat(flights.outgoingFlights);
        //         flightsSrvc.returnFlights.concat(flights.returnFlights);
        //         if(otherAirlines===1){
        //             $http.get('localhost:8100/api/other/flights/search/'+origin+'/'+destination+'/'+departingDate+'/'+returningDate+'/'+seatClass).success(function(othersFlights){
        //                 flightsSrvc.outgoingFlights.concat(othersFlights.outgoingFlights);
        ...
        // }
 		// $state.go('tabsController.findFlights.flights');
        flightsSrvc.outgoingFlights=[{ "flightNumber" : "RanaSE280472",
     "aircraftModel" : "a318",
      "aircraftType" : "Airbus",
      "capacity" : 60,
      "duration" : "2 hours",
      ...
      flightsSrvc.returnFlights=[{ "flightNumber" : "SomeoneSE280472",
     "aircraftModel" : "a318",
      "aircraftType" : "Airbus",
      "capacity" : 60,
      "duration" : "2 hours",
